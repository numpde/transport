...
with section("Loading OSM archive", print=print):
	...
	ways: pd.DataFrame
	(nodes, ways) = [
		pd.DataFrame(data=(x for x in J if (x['type'] == t))).set_index('id', verify_integrity=True).drop(
			columns="type")
		for t in ["node", "way"]
	]
...
with section("Making the graph", print=print):
	...
	nx.set_edge_attributes(G, name="len", values=dict(parallel_map(edge_len, G.edges)))
